...
def mode3(img_path='moonlanding.png'):
    ...
    sorted_flat_abs = np.sort(freq_abs.flatten())
    # Get the value of element in the X% place
    index10 = math.floor(sorted_flat_abs.size * 0.1)
    index30 = math.floor(sorted_flat_abs.size * 0.3)
    index50 = math.floor(sorted_flat_abs.size * 0.5)
    index75 = math.floor(sorted_flat_abs.size * 0.75)
    index95 = math.floor(sorted_flat_abs.size * 0.95)
    # If the value is under the threshold, we set it to 0
    filt10 = np.where(freq_abs > sorted_flat_abs[index10], freq, 0)
    filt30 = np.where(freq_abs > sorted_flat_abs[index30], freq, 0)
    filt50 = np.where(freq_abs > sorted_flat_abs[index50], freq, 0)
    filt75 = np.where(freq_abs > sorted_flat_abs[index75], freq, 0)
    filt95 = np.where(freq_abs > sorted_flat_abs[index95], freq, 0)
    # we take the inverse to get the image back
    filtimg10 = np.real(ifastft2(filt10))[:ih, :iw]
    filtimg30 = np.real(ifastft2(filt30))[:ih, :iw]
    filtimg50 = np.real(ifastft2(filt50))[:ih, :iw]
    filtimg75 = np.real(ifastft2(filt75))[:ih, :iw]
    filtimg95 = np.real(ifastft2(filt95))[:ih, :iw]

    # Display images
    plt.subplot(131)
    plt.imshow(image, cmap='gray'), plt.title('Compression 0')
    plt.subplot(132)
    ...
    print("Non zeros with 10% compression: ", np.count_nonzero(filt10))
    print("Non zeros with 30% compression: ", np.count_nonzero(filt30))
    print("Non zeros with 50% compression: ", np.count_nonzero(filt50))
    print("Non zeros with 75% compression: ", np.count_nonzero(filt75))
    print("Non zeros with 95% compression: ", np.count_nonzero(filt95))
